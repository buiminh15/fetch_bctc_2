...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://xaylaphue.com.vn/modules.php?name=News&op=viewcat&catid=40', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.HUB);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.HUB);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của HUB ::: ${name}`);
        })
      );
