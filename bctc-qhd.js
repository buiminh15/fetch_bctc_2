...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://qhd.com.vn/quan-he-co-dong', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(uniqNames, COMPANIES.QHD);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.QHD);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của QHD ::: ${name}`);
        })
      );
