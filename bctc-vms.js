...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('http://vimadeco.com.vn/quan-he-co-dong/', {
      headers: {
        'accept': 'text/html',
    ...
    console.log('📢 [bctc-vms.js:58]', uniqNames);
    const newNames = await filterNewNames(uniqNames, COMPANIES.VMS);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.VMS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của VMS ::: ${name}`);
        })
      );
