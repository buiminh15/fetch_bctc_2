...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://detmay7.com.vn/vn/tinh-hinh-tai-chinh.html', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.DM7);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.DM7);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của DM7 ::: ${name}`);
        })
      );
