...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://ciibr.com.vn/vi/quan-he-co-dong/bao-cao-tai-chinh/', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(uniqNames, COMPANIES.LGC);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.LGC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của LGC ::: ${name}`);
        })
      );
