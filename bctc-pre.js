...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.hanoire.com/vi/cong-bo-thong-tin', {
      headers: {
        'accept': 'text/html',
    ...
    $('a').each((_, el) => {
      ...
      const filterCondition = [currentYear, 'bctc'];
    ...
    const newNames = await filterNewNames(names, COMPANIES.PRE);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.PRE);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của PRE ::: ${name}`);
        })
      );
